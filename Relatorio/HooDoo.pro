...
solve(SolvedBoard,NrLines,NrColumns,TransparentMode):-
        ...
        statistics,
        !,
        labeling([minimize(Soma),down,ffc,bisect,time_out(120000,Flag)],TodasAsVars),
        statistics(runtime,Value),
        nl,
        write('Tempo decorrido :'),
        write(Value),
        nl,
        write('Pegs Transparentes Utilizadas:'),
        write(Soma),
        nl,
        fd_statistics,
...
test(Bi):-
        generateFlatList(Board,36),
        applyConstraints(Board,6,6,1),
        %sum(Board,#=,Soma),
        count(0,Board,#=,Soma),
        append(Board,[Soma],TodasAsVars),
        statistics,
